...
answer: passing data top-down the state tree from parent to child can be a headache, Context provides a way to share values without having to pass them all the way down the state tree.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

answer: the store is a single state tree where state cannot change(immutable) and all state changes are handled by dispatching actions. These actions are processed by reducers that accepts previous state and actions and returns the next state of your action.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?

answer: it is middleware that moves from a synchronous Redux action to a asyncronous flow where we can make API calls on action-creators

4. What is your favorite state management system you've learned in this sprint? Please explain why!

answer: Redux because it is thoughtful, well-documented, and predictable.
